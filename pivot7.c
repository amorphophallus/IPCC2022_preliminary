...
double SumDistance(const int k, const int n, const int dim, double* coord, int* pivots, double *redist){
    // double *idist = (double *) malloc(sizeof(double) * n * k);
    // for (int i = 0; i < n; i++) {
    //     for (int ki = 0; ki < k; ki++) {
    //         idist[i*n+ki] = redist[i*n+pivots[ki]];
    //     }
    // }
    ...
    for(int i=0; i<n; i++){
        int j;
        double idist[k];
        // idist[0]=redist[i*n+pivots[0]];
        // idist[1]=redist[i*n+pivots[1]];
        for (int l = 0; l < k; l++) {
        ...
        for(j=0; j<i; j++){
            ...
            for(ki=0; ki<k; ki++){
                ...
                // double dis = fabs(redist[i*n+pivots[ki]] - redist[j*n+pivots[ki]]);
...
void Combination(int ki, const int k, const int n, const int dim, const int M, double* coord, int* pivots,
    ...
    for(int i=pivots[ki-1]+1; i<n; i++) {
        pivots[ki] = i;
        Combination(ki+1, k, n, dim, M, coord, pivots, maxDistanceSum, maxDisSumPivots, minDistanceSum, minDisSumPivots, redist);
...
void FirstCombination(const int worldSize, const int worldRank, int ki, const int k, const int n, const int dim, const int M, double* coord, int* pivots,
                 double* maxDistanceSum, int* maxDisSumPivots, double* minDistanceSum, int* minDisSumPivots, double *redist){
    if(ki==k) {
    ...
    // } else {
    //     int osize = n / 6;
    //     size = (n - osize) % (worldSize - 1) ? ((n - osize) / (worldSize - 1) + 1) : ((n - osize) / (worldSize - 1));
    //     begini = osize + (worldRank - 1) * size;
    //     endi = MIN(osize + worldRank * size, n);
    // }

    // int size = (n % worldSize) ? (n / worldSize + 1) : (n / worldSize);
    // int begini = worldRank * size;
    // int endi = MIN((worldRank + 1) * size, n);
    ...
    for(int i=worldRank; i<n; i += worldSize) {
        ...
        // printf("rank = %d, i = %d, time = %f ms\n", worldRank, i, (end.tv_sec-start.tv_sec)*1000.0+(end.tv_usec-start.tv_usec)/1000.0);
        /** Iteration Log : pivots computed, best pivots, max distance sum, min distance sum pivots, min distance sum
        *** You can delete the logging code. **/
        // if(ki==k-2){
        //     int kj;
...
int main(int argc, char* argv[]){
    ...
    //     MPI_Send(redist, n*n, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    // } else {
    //     MPI_Recv(redist, n*n, MPI_DOUBLE, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    // }
    ...
    FirstCombination(worldSize, rank, 0, k, n, dim, M, coord, &temp[1], maxDistanceSum, maxDisSumPivots, minDistanceSum, minDisSumPivots, redist);
    ...
    MPI_Gather(maxDistanceSum, M, MPI_DOUBLE, maxDistanceSumAll, M, MPI_DOUBLE, 1, MPI_COMM_WORLD);
    MPI_Gather(maxDisSumPivots, k * M, MPI_INT, maxDisSumPivotsAll, k * M, MPI_INT, 1, MPI_COMM_WORLD);
    MPI_Gather(minDistanceSum, M, MPI_DOUBLE, minDistanceSumAll, M, MPI_DOUBLE, 1, MPI_COMM_WORLD);
    MPI_Gather(minDisSumPivots, k * M, MPI_INT, minDisSumPivotsAll, k * M, MPI_INT, 1, MPI_COMM_WORLD);
